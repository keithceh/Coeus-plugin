...
## 2026-Sep-09 (v3.26.0) · 2026-Sep-09 03:10 · keymaker v1.0.0 — coordinated end-to-end deliverables pass; nine deep-dive skill pages; every skill now documented

### Added — `keymaker` skill (family: tools, Tier 2)
The Keymaker opens every door in a project in one pass: a Tier-2 orchestration cluster chaining `project-lifecycle` (kickoff/resume/handover/audit/close) and `atlas` (the four evergreen visual outputs) over every project deliverable, with `evergreen-artefacts` publishing gated on request. Its own contribution is the sequencing, the ledger, and the quality pass — dependencies are launched via the Skill tool and **never reimplemented**:
- **The keyring** — Phase-1 deliverables ledger built at top tier from the atlas source-priority survey: one row per deliverable (owner skill · green/amber/red/unknown · gap · planned action); every row traces to a read file or an explicit user answer, unknowns marked `?`, never guessed. `plan-only` stops after the plan.
- **Model-tier orchestration** (minto precedent): PLAN and REVIEW inline at the session's top model (Fable-class, never delegated); independent deliverable builds to an **Opus** subagent; mechanical refreshes and doc regeneration to **Sonnet**; everything inline when the Agent tool or model overrides are unavailable — the keyring, not the delegation, is the deliverable.
- **Visual quality pass** (Phase 3, `skip-visual-pass` to skip) — every visualisation artefact checked against the new `references/quality_bar.md`: six graded sections (orientation, encoding honesty, direct labelling, typography/surface, self-containment/robustness, truthfulness) distilled from atlas `design_rules.md` + the artifact-design / visual-explainer / dataviz school, with honest `[V]`/`[H]` provenance markers and an explicit "design_rules.md wins on conflict" rule. Idempotent — an artefact already meeting the bar is untouched (zero bytes changed); self-containment outranks styling (an offline artefact never gains a CDN or webfont).
- **Loop until green** (Phase 4) — top-tier keyring re-walk running each dependency's own review gates; red or unaccepted-amber rows loop back, hard-capped at three full cycles, remaining reds reported honestly. Phase-5 report: keyring table, per-artefact bar findings, gated publish offer, §8 telemetry per lifecycle counting rules.
- Trigger surface: `/coeus:keymaker`, "keymaker", "full project pass", "end-to-end project pass", "coordinate all deliverables", "refresh every deliverable", "chain lifecycle and atlas", "sweep the project artefacts" — collision-audited: each phrase appears in exactly one skill description across the plugin. Never fires for a single deliverable (owner skills keep their doors).

### Changed — coeus-router v1.8.0
Keymaker row added FIRST in the Step-2b tools table (most specific multi-signal row; single-deliverable asks fall through by design), `keymaker` dependency registered, Step-1 tools family row extended with "an end-to-end pass over a whole project's deliverables". Tie-breaker 13's "visualise + update the handover" example rerouted from sequential atlas→lifecycle to `tools/keymaker` (rule 9's combo preference); new tie-breaker 14 (one deliverable → the owner skill, always; two-plus deliverable classes or an end-to-end/sweep ask → keymaker; a first-time single-artefact build is never keymaker). Golden set 31→35 (full-pass positive, combo reroute, single-deliverable border, named-skill BYPASS). To fit the 3,000-token cap the router shed ~130 words of non-normative prose (repeated "first match wins" lines consolidated, placeholder lines dropped, rationale sentences tightened) — zero routing behaviour changed; all 35 golden rows re-verified by hand. The file is structurally at capacity (~2,926 of 3,000): the next router addition pays the same trade.

### Validated — seer blind eval on the v1.8.0 router
24 scenarios synthesized from current frontmatter (forced coverage: all 18 routable skills in oracle; BYPASS / NO ROUTE / two-way-tie CLARIFIER / rule-9 sequential; borders for tie-breakers 4, 9, 13, 14), blind-routed by a subagent given only the router spec text and bare queries (genuine blindness), graded deterministic-first then the cascading rubric: **24/24 correct**, including the three critical keymaker borders (multi-deliverable → keymaker; "update the handover and give me a fresh project map" → keymaker; "just update the handover note" → project-lifecycle) and publish-with-atlas-word → evergreen-artefacts. One cosmetic finding (a "(first task)" annotation on a sequential FAMILY line) — no rule gap, no router edit. Disclosure per seer's hard rule: generator and judge share one model; agreement is not independent corroboration.

### Docs — every skill now carries a deep-dive page
Nine new pages in `docs/`, all grounded strictly in their SKILL.md/references sources with undated-version gaps flagged rather than invented: `Keymaker.md`, `Atlas.md`, `Seer.md`, `Minto.md`, `The-Architect.md`, `Morpheus-Pipeline.md` (morpheus + vendored prompt-master/caveman + the weekly-sync contract), `Plugin-Creator.md`, `Evergreen-Artefacts.md`, `Obsidian-Vault.md`. All nine wired into the landing page as Skills-tab companion panels (`build-index-html.py` SECTIONS/SUB_PARENT/SUB_LABEL/MD2TAB, same "← Back to Skills" mechanism as the EP-Council companions) and reachable via new "Deep dive:" links from every catalog entry in `docs/SKILLS.md`. Catalog header 18→19 skills, family map + tools family updated; `docs/Installation.md` counts 18→19 (3 places); `docs/Coeus-Architecture.md` diagram gains `keymaker*`; `docs/Tools.md` Related Pages gains Keymaker/Atlas/Evergreen links; `index.html` regenerated (343 KB) and now committed to the mirror. Handover skill table gains the keymaker row AND the previously missing `evergreen-artefacts` row (registration drift fixed).

### Registered
`SKILL_REGISTRY.md` keymaker row (tools, Tier 2, ~1,248 tokens) + tools family description; `coeus_full_test.py` OWNED += keymaker; `plugin.json` 3.26.0 with keymaker in the description; `marketplace.json` + `scripts/marketplace.json.template` stale "12-skill suite" description replaced with the current 19-skill list.

### Validation
`build-plugin.py --check-only` CHECK PASS 19 skills, 0 warnings; `coeus_full_test.py` PASS 257 / FAIL 0; both manifests parse as valid JSON; keymaker SKILL.md 938 words ≈ 1,248 tokens (cap 3,000); trigger-collision audit clean (every keymaker phrase unique plugin-wide); golden set 35/35 by hand; seer 24/24 blind.
...
Renders any project — codebase, report, deal, vault, field study — as a single offline HTML file a reader can open cold. Two-stage extract-then-render pipeline, the same split `dug_binary` already uses: `Outputs/atlas.json` (structured truth, schema in `references/schema.md`) → `Outputs/atlas.html` (rendered from `references/template.html` by replacing its `/*__ATLAS_DATA__*/` injection point). No CDN, no external font or image, no network fetch — it opens from a NAS share with the network off.

- **Four frozen regions, never reordered between versions.** ① FRAME (intent, scope in/out, guardrails each linked to the beat that created it, references) · ② NOW (the re-entry cue: where work stopped, open threads, next actions) · ③ MAP (enclosure-grouped SVG node-link graph, deterministic layered layout honouring `layout.group_order`/`node_order` — no force-directed physics, so the same JSON always renders the same picture; search ≥2 chars, click-to-pin with parent/child hues and the rest dimmed, hover enlarge, pan+zoom) · ④ STORY (causal beats — did X → because Y → hence Z — newest first, forks as branches, guardrail badges linking back to FRAME).
- **Evidence-graded doctrine, honestly flagged.** `references/design_rules.md` ranks 15 rules R1–R15 across three evidence tiers with effect sizes and `[V]`/`[V-cite]`/`[U]` verification markers, and carries the research pass's **verification caveat verbatim in substance**: full-text retrieval was blocked, every citation was verified against search-result metadata and abstract snippets only, and the lower figure is used wherever handbook and independent meta-analyses disagree. Seven mandatory uncertainty flags stay attached to the doctrine — dual-coding mechanism contested (justify text+image via search cost and distinctiveness, never two-channel theory); method-of-loci does **not** transfer-justify diagrams (R12 rests on spatial constancy and locational indexing, never "memory palace"); C4 is convention, not a validated finding; "big picture first" is small (d≈0.21–0.26) with one null direct domain test (Ernst & Robillard 2023) — **the atlas is a bet on mechanisms, not on diagram format**; chartjunk-vs-memorability reconciled as coherence-governs-content / memorability-governs-rendering (stated as inference); no learning-styles variants; no spaced-repetition machinery.
- **Never invents topology.** Phase 1 mines sources in priority order (lifecycle files → git repo → document folder → six-question interview) and every node, edge, and beat must trace to a mined source or an explicit user answer recorded in `meta.sources[]`. What cannot be established renders as a marked `?` placeholder with `status: unknown` — never guessed, never silently dropped.
- **Refresh preserves the layout.** A prior `atlas.json` is loaded first: node order, group order, and group assignment are inherited verbatim, new nodes append inside their group, story beats append, `now` is rewritten in full, and everything that moved is marked `delta: new|changed`.
- **Phase 3 review is always-on**: the design-rule checklist (verb on every edge, ≤5 hues with redundant shape/border, no legend, guardrails linked to beats, causal story, canonical region order) plus a self-containment grep. `llm-council` review stays gated to explicit request (minto's gating pattern). Phase 4 appends patterns-only lessons to `~/.coeus/atlas/learnings.md` with a ≥3-repeat fold-back proposal.
- **Borrowed patterns are cited, not laundered** (`design_rules.md` §Provenance): JSON-IR + validation gate + single-file output and the never-invent-topology rule from **archify**; expandable card detail levels and superseded/stale status from **thoughtdag**; deterministic-facts-then-semantic-summary and incremental refresh and layer colour-coding from **Understand-Anything**; stable spatial hierarchy and session transcripts as a Story source from **mempalace** (its benchmark/popularity claims are credibly challenged in a public audit and are deliberately not cited).

### Changed — coeus-router v1.7.0: atlas routing + tie-breaker 13
Step-1 tools row now catches "a visual map / at-a-glance explanation of a whole project"; Step-2b atlas row; tie-breaker 13 disambiguates the three skills that all touch "a project" — **the deliverable decides**: see it → `atlas`, operate the session (resume/handover/changelog/audit) → `project-lifecycle`, `.dugprj` or seismic volume lineage → `dug_binary` ("map volume lineage" is `dug_binary`, "map my project" is `atlas`); both → sequential per rule 9. Deps += atlas; golden set 29 → 31 (one positive atlas row, one "resume my project" border row guarding the lifecycle boundary). Golden set hand-checked: all 29 prior rows still route as labeled.

### Changed — project-lifecycle v1.4.1
One sentence in Sub-D and one appended step in `_shared/lifecycle_close.md`: after the artefacts index is regenerated, offer a Project Atlas refresh **only if `Outputs/atlas.html` already exists**. Lifecycle never creates an atlas and never refreshes one without the user's yes.

### Registration
SKILL_REGISTRY row (tools family, Tier 1, ~1,500 tokens, same commit per protocol) + tools-family definition updated; `scripts/coeus_full_test.py` OWNED += `atlas`; plugin.json 3.21.0 + suite description; README Skills-at-a-Glance row, slash-command row, and 16→17 skill counts; Coeus_LLM_HANDOVER.md header + skill table row + "What Coeus is" count.

### Docs drift fixes (2026-Sep-02)
`docs/Installation.md` skill counts 15→17 (stale since v3.19.0); `docs/Coeus-Architecture.md` golden set 29→31 / router v1.6.0→v1.7.0 / 15→16 routable skills, and the tier/family diagram gains the missing `seer` (drifted since v3.20.0) and new `atlas`; `docs/Project-Lifecycle.md` Version History gains the missing v1.4.0 row (drifted since v3.18.0) and the new v1.4.1 row; `index.html` regenerated via `scripts/build-index-html.py` (was 3 releases stale at "14 skills"; all 17 panels verified present post-regen).

---

## 2026-Aug-30 (v3.20.0) · New `seer` skill — spec-driven routing evaluation (Agent Seer protocol); coeus-router v1.6.0

### Added — `skills/seer/` (v1.0.0): routing-evaluation engine synthesized from skill specs
Adapts *Agent Seer: Synthesizing Scenarios from Specification Understanding* (Karumuri, Vemula & Lopes Pegna, Apple, arXiv 2608.26133) to Coeus's own tool surface: skill frontmatter is the tool specification, `coeus-router` is the agent under test, and the routing golden set is where confirmed failures become permanent regressions. Five-phase protocol, pure prompt/protocol design (no code, no live execution — the paper's technique is entirely inference-time):

- **Phase 0 grounds every constraint in context** — registry, router SKILL.md, golden set, and in-scope frontmatter loaded verbatim before anything is graded. Encodes the independent reproduction's headline lesson (ghchinoy/ai-paper-reproductions): the paper's judge scored a real unsupported-parameter bug a perfect 1.000 because the constraint lived outside the spec it was shown. A judge cannot penalize a rule it cannot see.
- **Phase 1 interprets each spec** into does / needs / used_for / family_signals / boundary_risks, grounded strictly in the frontmatter — spec-external "knowledge" of similarly named tools is banned (the paper's Git tool-name-hallucination failure mode).
- **Phase 2 synthesizes tiered scenarios** (simple/complex) with mandatory rationale fields (`route_reason`, `eval_value` — the paper's schema-mandated `quick_explanation`/`novelty_reason` pattern), coverage forced across every in-scope skill with a named-gap repair round (the paper's 100%-coverage mechanism on 6 of 7 MCP specs), gate/border/sequential cases required, and the intended route held out as oracle.
- **Phase 3 replays the router blind** — subagent with only the router SKILL.md when available; inline fallback is procedural blindness, disclosed as such in the report.
- **Phase 4 grades deterministic-first** (registry existence, block format, launch present, legal gate outcome — machine-checkable constraints never entrusted to judgment), then a decomposed rubric in `references/rubric.md`: 4 dimensions (Gate/Family/Skill/Protocol), 10 sub-dimensions, **cascading penalties** (wrong gate zeroes downstream; wrong family caps Skill+Protocol at 2) so one critical error collapses the record instead of averaging away — the paper's key metric contribution, which surfaced the failure class (argument value accuracy, 223/391 records) that coarse name-match metrics score as perfect.
- **Phase 5 reports + folds back**: per-family/tier scores, 10-label failure taxonomy, proposed golden-set rows and router-rule edits — proposals only, never silent edits. Every report carries the mandatory disclaimer *"Generator and judge share one model; agreement is not independent corroboration"* (the paper's judge-swap defense — Gemini vs Qwen — is unavailable to a single-context skill, so the honesty rule replaces the independence claim).

### Changed — coeus-router v1.6.0: seer routing + tie-breaker 12
Step-2a row (routing-eval verbs → `seer`), tie-breaker 12 (ask/explain about Coeus stays NO ROUTE per Step 0; test/evaluate/grade/synthesize routes `decision/seer` — the verb decides), deps + golden set grow 27 → 29 (one positive row, one ask-vs-test border row). Golden set hand-checked: all 27 prior rows still route as labeled.

### Fixed — drift
`scripts/coeus_full_test.py` OWNED set was missing `minto` (v3.19.0 shipped without adding it — version/argument-hint/Trigger-line enforcement was silently skipped for minto); added `minto` + `seer`. `Coeus_LLM_HANDOVER.md` skill table was missing the `minto` row; added alongside `seer`.

### Registration
SKILL_REGISTRY row (decision family, Tier 1, ~1,322 tokens, same commit per protocol); plugin.json 3.20.0 + suite description; README Skills-at-a-Glance/slash-commands/16-skill counts/router golden-set count; docs/Coeus-Architecture.md router paragraph (29 cases, 15 routable skills, seer as the in-house measurement of the catalog-degradation threshold); Coeus_LLM_HANDOVER.md header + table.

---

## 2026-Aug-28 (v3.19.0) · New `minto` skill — Pyramid Principle structuring engine; new **writing** router family

### Added — `skills/minto/` (v1.0.0): grounded Minto Pyramid Principle skill
Structures expository business communication per Barbara Minto's verified doctrine: answer-first pyramid under a single point, SCQ introduction, question/answer vertical logic, inductive Key Line, MECE groupings ordered by time/structure/degree, ban on intellectually blank headings. Design:

- **Grounded doctrine, not folklore.** `references/doctrine.md` distills three parallel deep-research passes (research date 2026-Aug-28), every claim tagged [P-DIRECT] (barbaraminto.com, her McKinsey alumni interview, HBS Class-of-1963 site) / [P-BOOK] (page-cited book text via named reproduction hosts) / [SECONDARY] (corroboration only — nothing originates there). Conflicts reported, not smoothed (edition history 1978→1987→1996→2002/2010; three-vs-four orders; story-order variant labels). Includes a **Myth Ledger** the skill must never repeat — headline: the widely circulated "Minto died in 2024" claim is unverified and apparently false (no obituary exists; name-matched obituaries are different people; Wikipedia lists her as living as of research date). Also corrected: "first woman at McKinsey" → verified claim is "first female MBA professional hire"; MECE is Minto's invention, not "a McKinsey framework"; her trademark is SCQ (SCQA/SCQR labels are derivative); first publication 1978, not 1985.
- **Auto-fire with a strict gate.** Fires only on explicit intent to structure/restructure/storyline an executive document or deck; hard NOT-trigger list defers to neighbours (llm-council/ep-council own the decision itself; project-lifecycle owns handovers/changelogs; ooxml-* own DOCX mechanics; prompt skills own prompts; external drafting skills own generic writing). Max one unsolicited offer per conversation. Manual trigger `/coeus:minto` always fires.
- **Tiered pipeline.** Phase 1 FRAME at top tier (never delegated) → skeleton gate for board/gate/proposal-class deliverables; Phase 2 BUILD delegates approved sections to Sonnet subagents, escalating high-stakes/failed sections to Opus, inline fallback when delegation unavailable; Phase 3 REVIEW layers pyramid QA (always) → `llm-council` (gated to high-stakes deliverables — delegated, never reimplemented) → top-tier read-as-the-reader pass; Phase 4 EVOLVE.
- **Self-evolution outside the repo.** Learnings data at `~/.coeus/minto/learnings.md` (trigger false-positives/misses, user preferences, accepted/rejected structures — patterns only, never content); `references/evolution.md` defines the fold-back protocol: lesson repeats ≥3× → propose folding into the skill as a normal version-bump commit. Doctrine changes only with new PRIMARY evidence; user preference adapts `references/playbooks.md`, never doctrine.
- **User-adapted playbooks.** `references/playbooks.md` maps the method onto the deliverables this user actually produces: board/investment recommendations, gate reviews (G1–G3), stakeholder updates (answer-first = BLUF with a derivation discipline), playback decks, proposals/framing docs, technical close-outs, emails, and document-surgery reviews.

### Changed — coeus-router v1.5.0: new **writing** family
Step-1 family row (structuring signals), Step 2e table (minto), tie-breaker 11 (writing vs decision: decision needed → decision family even if a document is the output; decision exists → minto; both → sequential per rule 9; session docs stay lifecycle), router deps + argument-hint updated. Golden set grows 22 → 27: two positive rows, two border rows (decision-vs-minto, lifecycle-vs-minto), one sequential row.

### Registration
SKILL_REGISTRY row + **writing** family definition (same commit, per protocol); plugin.json 3.19.0 + suite description; README Skills-at-a-Glance/slash-commands/family notes; docs/Coeus-Architecture.md family list; Coeus_LLM_HANDOVER.md header.

---

## 2026-Jul-12 (v3.18.1) · Upstream caveman contract alignment audit

### Audited — all caveman-consuming skills vs JuliusBrussee/caveman@main
Local `skills/caveman/` verified byte-identical to upstream (modulo the pinned `name:`/`argument-hint:` sync normalization — by design). Consumer audit against the current upstream contract (which now BANS invented abbreviations like cfg/impl/fn and causal arrows at every level — measured zero token saving under the tokenizer, added decode cost):

- **`skills/caveman/README.md` (Coeus wrapper, not synced)** — ultra row still described the OLD spec ("Abbreviations (DB/auth/cfg/fn/impl), causality arrows (X → Y)"). Rewritten to current upstream: strip conjunctions where unambiguous, one word when one word enough, NO invented abbreviations, NO arrows, standard acronyms OK.
- **`morpheus` Step-2 Compression Rules** — added rule 8: upstream token rules apply to the compressed prompt (standard acronyms OK; no invented abbreviations; no causal arrows).
- **`the-architect` Step A2 inherited rules** — added rule 5, same contract.
- **Coeus-specific additions confirmed non-conflicting and retained:** prompt-only scope rule (upstream is a response-style skill; prompt-compression is a Coeus application), preserve-format-markers / uncertainty-instructions / output-format-specs rules, auto-disable under 150 tokens and for o3/o1, artifacts-never-compressed guardrail.

No changes to `skills/caveman/SKILL.md` itself — it tracks upstream via weekly sync and release-time fetch; wrapper docs are the only Coeus-owned surface, now aligned.

---

## 2026-Jul-11 (v3.18.0) · morpheus AUTO/REVIEW modes, lifecycle handover rename-on-resume, stale-doc purge

### Changed — morpheus v1.2.0: Review Option (checked before pre-flight)
Default is AUTO: engineer, compress, execute in one pass (auto-execute shipped in v1.1.0; now the mode is explicit and stated at the top of the output block). REVIEW mode is opt-in by phrasing at invocation ("review first", "show me the prompt before running", --review): pipeline runs Steps 1-2, delivers the prompt, stops, and executes only on an explicit "run". Never asks which mode - AUTO unless the user opted in. REVIEW added as first Auto-Disable condition for Step 3.

### Changed — project-lifecycle v1.4.0: handover rename-on-resume (resume step 7)
On resume, if the handover filename does not match `<task name>_handover_note.md`, rename it (task name from the handover title or project dir, spaces to underscores), update every reference to the old filename across the project Outputs/ tree, report any hits in files that must not be edited (e.g. protected DOCX), and log the rename in the changelog. Rename, never duplicate - three-core-files contract holds.

### Fixed — handover stale references purged (refactor sweep)
"How to ship a release" still described the retired bundle layout (commands/, skill_downloader.py); troubleshooting and open-work rows still pointed at the retired skill_downloader OAuth flow. Corrected to canonical layout; open-work row now points at the public-mirror install paths that superseded the downloader. Sweep found no other dead refs: no root plugin.json, no tracked __pycache__, hooks clean.

---

## 2026-Jul-11 (v3.17.1) · coeus-router v1.4.0 — gate, sequential routing, tiered fallback, golden set

Evaluated the router against two 2025-26 references on agent tool selection (RAG-MCP / MCPToolBench++ lineage: gating, retrieval, routing, planning, fallback, benchmarking) and the tools-vs-subagents decision framework. Verdict: the two-step family-first design IS the recommended semantic-routing pattern, and at 13 routable skills the catalog sits well under the ~15-20-candidate degradation threshold — so vector-store retrieval was evaluated and deliberately skipped (over-engineering at this scale; revisit if the catalog passes ~20 skills or any single family passes ~10). Launching skills in-context via the Skill tool is the correct tool-like contract vs a subagent (no context isolation needed; the routed-to skill needs the full conversation).

### Added — Step-0 Gate (router v1.4.0)
Three outcomes before any family matching: BYPASS (user named the skill — launch directly, no routing block), NO ROUTE (no Coeus skill applies — say so and answer normally; never force a route), PROCEED. Closes the "router always picks something" failure mode.

### Added — Sequential cross-family routing (tie-breaker 9)
"Audit this DUG project, then write the handover" now routes + launches the first task's skill and names the second in the WHY line as an explicit handoff. One launch per response; combo skills (morpheus, the-architect) preferred over manual sequencing within a family.

### Added — Three-tier confidence fallback (tie-breaker 10)
(a) clear signal: route + launch; (b) two-way tie: one clarifier naming both, max one; (c) no signal: NO ROUTE, never a guess.

### Added — Routing golden set (`skills/coeus-router/references/routing-golden-set.md`)
22 labeled query-to-outcome pairs covering every family, every tie-breaker, gate outcomes, sequential routing, and no-signal cases. Doubles as a hand-run regression checklist (pinned by a Hard Rule in the SKILL.md) and a few-shot anchor. Maintenance rule: new skill = at least one positive row + one border row.

---

## 2026-Jul-10 (v3.17.0 follow-up) · Canonical update command — marketplace clone gotcha

### Fixed (docs + reminder text) — "still on old version after reinstall"
Root cause diagnosed: Claude Code/Desktop installs plugins from a LOCAL git clone of the marketplace (`~/.claude/plugins/marketplaces/coeus`), refreshed only by `/plugin marketplace update coeus` — never by reinstall, remove+re-add, app restart, or machine restart. A user stuck on v3.13 while the mirror served v3.17.0 traced to this clone frozen at its install-day snapshot. Client behaviour is Anthropic-side and not fixable from this repo; every update instruction we ship now uses the canonical two-step command:

```
/plugin marketplace update coeus && /plugin update coeus
```

Updated: README (Option 2 + installer section, with a "why two commands" callout), docs/Installation.md (update section + gotcha callout), scripts/mirror-workflows/release-reminders.yml (7-day reminder text), Coeus_LLM_HANDOVER.md (marketplace-path section + install-paths list). Manual recovery for an already-stale clone: `git -C ~/.claude/plugins/marketplaces/coeus fetch origin && git -C ~/.claude/plugins/marketplaces/coeus reset --hard origin/main` (plain pull fails — the mirror is force-pushed).
