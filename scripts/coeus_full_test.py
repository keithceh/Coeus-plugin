...
OWNED = {'ep-council', 'llm-council', 'morpheus', 'the-architect', 'plugin-creator',
         ...
         'dug_binary', 'obsidian-vault', 'minto', 'seer'}
VENDORED = {'caveman', 'prompt-master'}

# Description char cap: single source of truth is build-plugin.py's
# DESCRIPTION_CHAR_CAP (the build gate). Parse it so the two can't drift again
# (they did: this file asserted the stale 1024 cap after the build tightened to 500).
DESC_CAP = int(re.search(
    r'^DESCRIPTION_CHAR_CAP\s*=\s*(\d+)',
    (pathlib.Path(__file__).resolve().parent / 'build-plugin.py').read_text(encoding='utf-8'),
    re.M).group(1))
...
def test_skills():
    ...
    for skdir in sorted((ROOT / 'skills').iterdir()):
        ...
        # description char cap (sourced from build-plugin.py DESCRIPTION_CHAR_CAP)
        if len(desc) > DESC_CAP:
            bad(f'{name}: description ≤ {DESC_CAP} chars',
                f'has {len(desc)} — build-plugin.py check_skills() will reject')
        else:
            ok(f'{name}: description ≤ {DESC_CAP} chars ({len(desc)})')
