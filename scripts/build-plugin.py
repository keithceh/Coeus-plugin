...
DESCRIPTION_CHAR_CAP = 1024  # Cowork SKILL.md description field validator limit


def check_skills() -> int:
    """Validate skill-architecture invariants. Returns nonzero on hard failure.

    Hard fails: registry parity, token cap (non-vendored), frontmatter name match.
    Warnings (escalate to hard fail in v3.9): router coverage.
    """
    ...
    for d in skill_dirs:
        ...
        # router coverage (WARNING until v3.9; coeus-router itself excluded)
        if name != 'coeus-router' and rtr_text:
            # match `<name>` in any table row OR `/coeus:<name>` slash form
            if f'`{name}`' not in rtr_text and f'/coeus:{name}' not in rtr_text:
                warnings.append(f'{name}: not in coeus-router routing table')
