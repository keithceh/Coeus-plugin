...
| `coeus-router` | **Tier-3 meta-skill** — the Coeus guru / skill selector: routes the user's intent to the right Coeus skill and launches it via the Skill tool in the same response, no domain logic (v1.2.0) | `/coeus:router` |

`morpheus` and `the-architect` depend on `prompt-master` + `caveman`. `ep-council`, `plugin-creator`, `dug_binary`, and all four Tools skills are standalone.
