...
| `atlas` | **Tools family** — Project Atlas generator: mines a project (lifecycle files, git repo, document folder, or a six-question interview) into `Outputs/atlas.json`, then renders one self-contained offline `Outputs/atlas.html` — four frozen regions (Frame / Now / Map / Story), enclosure-grouped node-link map with verbs on every edge, search + click-to-pin + pan/zoom, causal history with forks and guardrail badges, delta markers on refresh. Never invents topology; unknowns render as marked placeholders. Evidence-graded doctrine in `references/design_rules.md` (added v3.21.0) | `/coeus:atlas` |
