...
**Current version**: 3.26.0 (2026-Sep-09 — new `keymaker` skill v1.0.0: Tier-2 tools-family orchestration cluster chaining `project-lifecycle` and `atlas` into ONE coordinated pass over every project deliverable — the **keyring** ledger (one row per deliverable: owner skill, green/amber/red/unknown state, gap, planned action; every row traces to a read file or a user answer, unknowns marked `?`), dependencies launched via the Skill tool and never reimplemented, evergreen-artefacts publishing gated on request; model-tier orchestration per the minto precedent (Phase 1 PLAN + Phase 4 REVIEW inline at top tier/Fable-class, independent builds to an Opus subagent, mechanical refreshes to Sonnet, inline fallback when Agent tool unavailable); Phase 3 visual-quality pass sweeps every visualisation artefact against `skills/keymaker/references/quality_bar.md` (6 graded sections distilled from atlas design_rules + the artifact-design/visual-explainer/dataviz school, [V]/[H] provenance markers; idempotent — passing artefacts untouched; self-containment outranks styling), loop-until-green with a hard 3-cycle cap and remaining reds reported honestly; coeus-router v1.8.0 adds the Step-2b keymaker row FIRST in the tools table (most specific multi-signal row, single-deliverable asks fall through), rewires tie-breaker 13's example to the combo skill and adds tie-breaker 14 (one deliverable → its owner, two-plus classes or an end-to-end/sweep ask → keymaker), golden set 31→35; router trimmed ~130 words of non-normative prose to fit the 3,000-token cap (now ~2,926) — all 35 golden rows re-verified by hand; seer blind eval run on the 1.8.0 router: 24 synthesized scenarios covering all 18 routable skills + BYPASS/NO-ROUTE/CLARIFIER/sequential + tie-breaker-9/13/14 borders, blind-routed by a spec-only subagent, 24/24 correct (single-model generator/judge disclosed); docs: nine new deep-dive pages (Keymaker, Atlas, Seer, Minto, The-Architect, Morpheus-Pipeline, Plugin-Creator, Evergreen-Artefacts, Obsidian-Vault) wired into the landing page as Skills-tab companion panels with Deep-dive links from the catalog, SKILLS.md 18→19, Installation counts 18→19, marketplace.json stale '12-skill suite' description fixed to the 19-skill list, index.html regenerated and committed. Prior: 3.25.0 (2026-Sep-08 — atlas v1.3.0: fourth evergreen output `Outputs/atlas_toc.html` — the visual Contents hub, a clickable table of contents linking every artefact in one place: inline SVG ecosystem explainer (atlas.json renders→atlas.html, derives→MoC + ToC, ToC links→every artefact; "you are here" marked), ten quick-link chips (atlas + four region anchors + three `#view=` Map views + MoC + atlas.json), by-family cards on the validated palette + a by-kind second cut with fragment cross-links, optional ≥2-char filter, fully readable with JS off (asserted by parsing, not executing); `meta.toc` data-driven "Contents" link in atlas.html and a MoC header link, both backward-compatible; evergreen gate now four files created once/updated in place; contract in schema.md § Table of Contents; demo (refreshed to 27 nodes incl. evergreen-artefacts, 38 edges, 14 beats) validated 224 jsdom + 340 real-Chromium assertions incl. real pointer/keyboard input and a byte-identical second run; docs/SKILLS.md atlas entry corrected to Tier 1 (registry is canonical). Prior: 3.24.1 (2026-Sep-08 — docs/SKILLS.md 18-skill catalog + Skills tab; landing-page visual revamp in build-index-html.py (type scale, zebra tables, underline nav, reduced-motion); nas-artefacts renamed evergreen-artefacts across the plugin; single-branch policy (all side branches merged + deleted). Prior: 3.24.0 evergreen-artefacts v1.0.0. Prior: 3.23.1 (2026-Sep-04 — atlas v1.2.1 fix: Overview group boxes never received real mouse clicks (pointer captured on pointerdown retargeted the click to the wrapper; capture now waits for actual drag movement >4px), so member-panel/ego drill-in was mouse-unreachable in v3.22.0–v3.23.0 while synthetic dispatchEvent-driven tests passed; render check + review checklist now mandate real pointer input where a driver exists. Prior: 3.23.0 (2026-Sep-04 — atlas v1.2.0: third evergreen output `Outputs/atlas_moc.md` — a Master of Content markdown directory of every artefact (contract in `skills/atlas/references/schema.md` § Master of Content: by-group + by-kind cuts each listing every map node exactly once, beats with forks marked, guardrails with origin beats, sources; project-root-relative `nodes[].link` values rewritten with `../` because the MoC lives in `Outputs/` — documented as its own schema rule), data-driven "Master of Content" header link in atlas.html (only when `meta.moc` set — old files render unchanged), Map views hash-addressable (`#view=overview|list|grid`); plus the evergreen create/refresh gate: if `Outputs/atlas.json` exists the run IS a refresh even when invoked as create, three outputs created once and updated in place forever, versioned/timestamped siblings are a defect (Phase-3 output check + Hard Rule; protocol-enforced, not code — future `scripts/check_outputs()` noted); demo validated 152 jsdom + 246 Chromium assertions incl. a simulated second run leaving the output directory byte-identical. Prior: 3.22.0 (2026-Sep-04 — atlas v1.1.0 Map rework: the single full node-link graph is replaced by a three-view composite chosen via a six-way bake-off on this repo's own atlas data (three interactive renderings vs the three best seaborn statistical forms, all validated headless + real Chromium) — Overview (group-level enclosures, ONE aggregated verb-labelled edge per group pair, ~5 boxes/~8 edges, default), List (relation tree, every edge a named chip, print form), Grid (native-SVG group matrix with empty cells drawn — the negative-space payoff from the seaborn heatmap, no Python at read time), with member-panel → ego-view drill-in (in-place group expansion built, tested, REJECTED: expansions containing a hub's targets reproduce the full-graph occlusion — coeus-router's 16 routes-to edges, established empirically 2026-Sep-02); v1.0.0's raw hsl group hues failed CVD validation (adjacent-pair ΔE 5.2 vs ≥8 target) and are replaced by a fixed validated five-slot palette assigned by group_order position, numeric layout.hues now advisory-ignored (null = neutral still honoured); design_rules.md gains a Bake-off addendum recording single-dataset empirical results incl. the clustering negative result (cosine clustering recovers declared families at purity 54% / ARI 0.05 — families are human judgment, not link structure); demo validated 96 jsdom + 181 Chromium assertions, harness 232/232, three browser-only defects caught and fixed. Prior: 3.21.0 (2026-Sep-01 — new `atlas` skill v1.0.0: Project Atlas generator — mines any project (lifecycle files → git repo → document folder → six-question interview) into `Outputs/atlas.json` (schema in `skills/atlas/references/schema.md`) and renders ONE self-contained offline HTML from `references/template.html` at the `/*__ATLAS_DATA__*/` injection point; four frozen regions (FRAME / NOW / MAP / STORY), enclosure-grouped SVG node-link map with a deterministic layered layout and a verb on every edge, search + click-to-pin + hover-enlarge + pan/zoom, causal story beats with forks and guardrail badges, delta markers on refresh, prefers-color-scheme themes, print stylesheet, `<noscript>` + `window.onerror` hardening; never invents topology — every node/edge/beat traces to a mined source or an explicit user answer and unknowns render as marked `?` placeholders; refresh inherits node/group order verbatim so the layout never moves; evidence-graded doctrine in `references/design_rules.md` (15 ranked rules, effect sizes, [V]/[V-cite]/[U] markers, the full-text-blocked verification caveat, and seven mandatory uncertainty flags — the atlas is a bet on mechanisms, not on diagram format); coeus-router v1.7.0 adds the atlas row + tie-breaker 13 (the deliverable decides: see it → atlas, operate the session → project-lifecycle, .dugprj → dug_binary), golden set 29→31; project-lifecycle v1.4.1 + `_shared/lifecycle_close.md` step 12 offer an atlas refresh only when `Outputs/atlas.html` already exists. Prior: 3.20.0 (2026-Aug-30 — new `seer` skill v1.0.0: spec-driven routing evaluation adapted from the Agent Seer paper (arXiv 2608.26133) — synthesizes labeled routing scenarios from skill frontmatter alone with forced coverage + named-gap repair, holds the intended route as hidden oracle, replays coeus-router blind (subagent when available), grades deterministic-checks-first then a cascading decomposed rubric (`skills/seer/references/rubric.md`), proposes golden-set rows and router-rule edits (never silent edits), mandatory single-model disclaimer in every report; coeus-router v1.6.0 adds the seer row + tie-breaker 12 (ask/explain → NO ROUTE, test/evaluate/grade → seer), golden set 27→29; drift fixes: coeus_full_test.py OWNED += {minto, seer}, this table gained the missing minto row. Prior: 3.19.0 (2026-Aug-28 — new `minto` skill v1.0.0: Pyramid Principle structuring engine on verified Barbara Minto doctrine (three tagged-grounding research passes; Myth Ledger incl. the false "died 2024" claim), auto-fire gate with hard NOT-trigger list, tiered Fable-plan/Sonnet-draft/Opus-escalate pipeline, gated llm-council review, self-evolution data at `~/.coeus/minto/learnings.md` with a ≥3-repeat fold-back protocol; coeus-router v1.5.0 adds the **writing** family, tie-breaker 11, golden set 22→27. Prior: 3.18.1 (2026-Jul-12 — upstream caveman contract audit: wrapper README ultra row fixed to current upstream (no invented abbreviations, no arrows), morpheus rule 8 + architect A2 rule 5 added; all Coeus prompt-scope additions confirmed non-conflicting. Prior: 3.18.0 (2026-Jul-11 — morpheus v1.2.0 AUTO/REVIEW modes (AUTO default: engineer+compress+execute one pass; REVIEW opt-in via "review first"/--review, stops for explicit "run"); project-lifecycle v1.4.0 resume step 7 renames handover to <task>_handover_note.md + updates all refs; handover stale-ref purge (retired commands/ + skill_downloader.py). Prior: 3.17.1 (2026-Jul-11 — coeus-router v1.4.0: Step-0 gate (BYPASS/NO ROUTE/PROCEED), sequential cross-family routing, three-tier confidence fallback, routing golden set at skills/coeus-router/references/routing-golden-set.md — evaluated against RAG-MCP-lineage tool-selection guidance; vector retrieval deliberately skipped at 13 skills, revisit past ~20. Prior: 3.17.0 (2026-Jul-10 — obsidian-vault v1.2: full obsidian-mcp parity verified from the package source (soft delete to `.trash/` + trash_metadata, vault-wide link updating on move + broken-backlink reporting on delete, move hardening, prepend, filename + hierarchical/wildcard tag search, tag semantics — specs in `skills/obsidian-vault/references/OPERATIONS.md`) plus 8 security guardrails from a prompt-injection/path-containment review (`.obsidian/` hard write block, resolved-path containment, regex-escaped link rewriting, live-reply-only consent, `.trash/` writes gated as delete family, wildcard disclosure). Prior: 3.16.0 — new `obsidian-vault` skill: direct Obsidian-vault file operations (read/search/create/edit/tag/move/delete notes) on plain-text vaults incl. NAS/UNC paths, via native file tools or the filesystem MCP server, with per-type confirmation gating for destructive ops; starts the new **vault** router family. coeus-router v1.3.0 adds Step 2d routing to `obsidian-vault`. Prior: 3.15.8 — coeus-router v1.2.0: router must LAUNCH the routed-to skill via the Skill tool, not just print the `RUN →` slash command (printed slash commands are inert); "morph this" added to morpheus triggers + router morpheus row; guru/skill-selector framing. Prior: 3.15.7 — EP-Council ground-truth protocol COMPLETE: all 9 member profiles Perplexity-verified claim-by-claim across v3.15.2–3.15.7; zero `[UNCONFIRMED]` markers remain in the Members doc. Highlights: BP dual ouster (CEO Dec 2025, chair May 2026), Willow remand-without-vacatur nuance, Guyana capacity corrected to ~1 Mb/d-by-2027, OxyChem close 2 Jan 2026, Hollub succession Mar 2026. Prior: 3.15.1 dug_binary patch) — note: the header had drifted to 3.13.0 while v3.14.0 (Seismic Tools v2.0, `dug_projdb` → `dug_binary`) shipped without a header bump; CHANGELOG.md is authoritative for that release
...
Coeus is a **Claude Code plugin** that bundles 19 skills for high-stakes EP-industry decision-making, plugin authoring, meta-routing, upstream subsurface data extraction, direct Obsidian-vault file operations (the **vault** family, v3.16.0), Minto pyramid-principle document structuring (the **writing** family, v3.19.0), Project Atlas generation (`atlas`, v3.21.0), evergreen NAS publishing (`evergreen-artefacts`, v3.24.0), and the Keymaker coordinated end-to-end deliverables pass (`keymaker`, v3.26.0):
...
| `atlas` | **Tools family** — Project Atlas generator: mines a project (lifecycle files, git repo, document folder, or a six-question interview) into `Outputs/atlas.json`, then renders one self-contained offline `Outputs/atlas.html` — four frozen regions (Frame / Now / Map / Story). Map v1.1.0 = three views: Overview (group-level, one aggregated verb-labelled edge per group pair — default), List (relation tree, print form), Grid (group matrix, empty cells shown), with member-panel → ego-view drill-in, search in all three, and a fixed validated five-slot palette assigned by group_order position (raw v1.0.0 hues failed CVD validation). Causal history with forks and guardrail badges, delta markers on refresh. Never invents topology; unknowns render as marked placeholders. Evidence-graded doctrine + bake-off addendum in `references/design_rules.md`. v1.2.0: third evergreen output `Outputs/atlas_moc.md` (Master of Content — markdown directory of every artefact, by-group + by-kind cuts, `#view=` deep links) + create/refresh gate: create on an existing atlas auto-refreshes in place, no versioned copies ever v1.3.0: fourth evergreen output `Outputs/atlas_toc.html` — visual Contents hub (clickable ToC of every artefact, inline SVG ecosystem explainer, quick links into all regions/views, by-family + by-kind cuts, optional filter, no-JS-complete; contract in schema.md § Table of Contents) (added v3.21.0; Map rework v3.22.0; MoC + evergreening v3.23.0; visual ToC v3.25.0) | `/coeus:atlas` |
| `evergreen-artefacts` | **Tools family** — agent-neutral evergreen publishing to a Synology NAS: stable evergreen link per artefact redirecting to the latest immutable `vN/`, `publish.json` per version (publisher, ISO timestamp, per-file SHA-256), revert republishes old content as a NEW version with `revertOf` recorded — nothing ever deleted (added v3.24.0; this table row added v3.26.0 fixing a registration drift) | `/coeus:evergreen-artefacts` |
| `keymaker` | **Tools family** — Tier-2 orchestration cluster: chains `project-lifecycle` + `atlas` over every project deliverable via the keyring ledger, launches dependencies through the Skill tool (never reimplements), sweeps all visualisation artefacts against `references/quality_bar.md` idempotently, top-tier plan/review with Opus/Sonnet delegated execution, loop-until-green with a 3-cycle cap and honest reds (added v3.26.0) | `/coeus:keymaker` |
...
| `minto` | **Writing family** — Pyramid Principle structuring engine on verified Barbara Minto doctrine (answer-first pyramid, SCQ, MECE, inductive key line); tiered draft pipeline, gated llm-council review, self-evolution data at `~/.coeus/minto/learnings.md` (added v3.19.0) | `/coeus:minto` |
| `seer` | Spec-driven routing evaluation — Agent Seer protocol (arXiv 2608.26133): scenarios synthesized from skill frontmatter with forced coverage, oracle held out, blind router replay, deterministic checks then cascading decomposed rubric, golden-set fold-back proposals (added v3.20.0) | `/coeus:seer` |
...
`morpheus` and `the-architect` depend on `prompt-master` + `caveman`; `minto` depends on `llm-council` (gated Phase-3 review). `ep-council`, `plugin-creator`, `dug_binary`, `obsidian-vault`, and all four Tools skills are standalone.
...
4. Create a GitHub Release on that tag. `release.yml` fires automatically:
   ...
   - Builds `coeus.plugin` containing `.claude-plugin/`, `skills/`, `hooks/`, `scripts/`, and the wrapper docs (canonical layout since v3.2; `commands/` and `skill_downloader.py` retired).
...
- (resolved) The retired `skill_downloader.py` OAuth friction is superseded by the public mirror `keithceh/Coeus-plugin` — non-collaborators install via marketplace or GitHub-direct.
