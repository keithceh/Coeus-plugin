...
**Current version**: 3.13.0 (2026-Jul-02)
...
Coeus is a **Claude Code plugin** that bundles 13 skills for high-stakes EP-industry decision-making, plugin authoring, meta-routing, and upstream subsurface data extraction:
...
| `ep-council` | 9-supermajor E&P adversarial decision engine; v1.9 — 13 traps (T0–T12), Shell carries T4 + T10, Chevron carries T2 + T11, ExxonMobil carries T6 + T12 | `/coeus:ep-council` |
...
| `dug_projdb` | **Seismic Tools family** — reverse-engineers a DUG Insight `project.dugprj` into a multi-sheet xlsx (volumes, horizons, polygons, per-volume process history) (added v3.10.0) | `/coeus:dug_projdb` |
| `coeus-router` | **Tier-3 meta-skill** — routes the user's intent to the right Coeus skill, no domain logic | `/coeus:router` |

`morpheus` and `the-architect` depend on `prompt-master` + `caveman`. `ep-council`, `plugin-creator`, `dug_projdb`, and all four Tools skills are standalone.
