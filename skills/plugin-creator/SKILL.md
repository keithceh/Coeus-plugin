...
description: >
  Trigger on: /plugin-creator, "plugin this", "make plugin", "autobot transform",
  "package this as a plugin", "turn this into a plugin", "wrap this as a Claude
  plugin", "make a .plugin", "scaffold a plugin", or any time the user wants to
  convert one or more skills/prompts into an installable Claude plugin bundle.

  Autobot transformer — converts a raw idea, a single prompt, a workflow note, or an
  existing folder of skills into a spec-compliant Claude Code / Cowork plugin ready for
  desktop "Add Plugin" install. Encodes the full anthropics/knowledge-work-plugins
  standard plus the hard-won lessons from the Coeus v3.0 → v3.4.0 conformance pass:
  canonical 4-key manifest, per-skill SKILL.md auto-registration, optional
  SessionStart cleanup hook for clean in-place upgrades, reproducible build scripts,
  UTF-8/LF/no-BOM hygiene, and a conformance check that can fail.

  Lean toward triggering whenever the user has produced a working prompt or skill
  and wants it shareable/installable rather than copy-pasted.
dependencies: []
---
