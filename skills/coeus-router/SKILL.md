...
description: >
  Trigger on: /coeus:router, /coeus-router, "which coeus skill", "route this",
  "pick the right coeus skill", "what coeus skill should I use", "coeus help me decide",
  "i'm not sure which coeus", "i am not sure which coeus", "not sure which coeus".

  Tier-3 meta-skill router for the Coeus plugin. Reads the user's intent and
  delegates to the correct Tier-1 or Tier-2 Coeus skill across both families:
  decision (llm-council, ep-council, morpheus, the-architect, prompt-master,
  caveman, plugin-creator) and tools (ooxml-repair, ooxml-fields, docx-inventory,
  project-lifecycle). Contains NO domain logic of its own — it only routes.
  Uses a two-step decision: family first, then skill within family. Use when
  the user knows they want a Coeus skill but does not know which one, or when
  intent spans multiple skills and a single entry point is wanted.

  Do NOT use when the user has already named a specific skill — invoke that one
  directly. Do NOT use for arbitrary chat questions outside Coeus's scope.
dependencies:
  - llm-council
  - ep-council
  - morpheus
...
If both signals are present, the more concrete artefact wins (a file path or
a DOCX always beats a generic word like "decision"). If neither is clearly
present, ask one clarifier and stop — do not guess.
...
7. **Ambiguity:** If two skills tie after all rules above, name both in a one-line clarifier and let the user pick. Do not invent a hybrid.
