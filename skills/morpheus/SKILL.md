...
description: >-
  Trigger on /morpheus or /morph, "morph this", "morph it", "engineer this prompt", "optimise this prompt", "make this prompt better", "refine my prompt for", "compress this prompt".
...
Also activate if the user says: "morph this", "morph it", "engineer this prompt",
"optimise this prompt", "compress this prompt", "make this prompt better",
"refine my prompt for [model]" — with or without a leading slash.

---

## Review Option (v1.2.0 — checked FIRST, before pre-flight)

Default behaviour is **auto-execute**: the pipeline engineers, compresses, and
runs the prompt in one pass (Step 3). The user can opt into a review gate at
invocation time:

- If the request contains "review first", "let me review", "show me the prompt
  before running", "check before run", `--review`, or similar → set **REVIEW
  mode**: run Steps 1–2, deliver the Morpheus Output block, then STOP with one
  line — `Review mode: reply "run" to execute, or edit the prompt and resend.`
  Execute only on explicit go-ahead.
- Anything else → **AUTO mode** (default): proceed straight through Step 3.

State the active mode in one word at the top of the output block (`Mode: AUTO`
or `Mode: REVIEW`). Never ask "which mode do you want?" — the default is AUTO;
review is opt-in by phrasing, not by questionnaire.
...
8. **Upstream caveman token rules apply** — standard acronyms (DB/API/HTTP) OK; never invent abbreviations (cfg/impl/req/res/fn) and never use causal arrows (X → Y) in the compressed prompt: the tokenizer splits both the same as full words — zero tokens saved, decode cost added
...
- REVIEW mode is active (see Review Option) — deliver, stop, await explicit "run"
...
| Phrase | Action |
|---|---|
| `/morpheus`, `/morph`, "morph this", "morph it" | Full pipeline |
