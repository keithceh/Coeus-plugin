...
12. **Offer a Project Atlas refresh — only if `Outputs/atlas.html` already exists.** Step 3 changed the artefacts index, so the atlas is now stale: add one line to the close report offering `/coeus:atlas refresh`. Never create an atlas that does not already exist, and never refresh without the user's yes.
...
- [ ] Project Atlas refresh offered **if** `Outputs/atlas.html` exists (never created here)
