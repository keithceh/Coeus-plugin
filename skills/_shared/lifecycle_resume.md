...
7. **Normalize the handover filename to the task name.**
   If the handover note's filename does not already match the pattern
   `<session/task name>_handover_note.md` (task name from the handover's own
   title or the project directory name, spaces → underscores):
   a. Rename the file to `<task name>_handover_note.md` (same directory).
   b. Grep the project's Outputs/ tree (and the handover itself) for links or
      references to the OLD filename; update every hit to the new name.
   c. If any reference lives in a file you may not edit (e.g., a protected
      DOCX), report it instead of editing.
   d. Record the rename as a one-line entry in the changelog.
   Skip silently if the name already conforms. Never create a second copy —
   this is a rename, not a duplicate (three-core-files contract: one handover,
   updated in place).
