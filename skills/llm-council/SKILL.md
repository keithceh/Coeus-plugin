...
description: >
  Trigger on: /council, "Jedi", "May 4th", "run the council", "stress-test this plan",
  "red team this", "premortem", "multi-model analysis", "get all the AIs to weigh in",
  "simulate expert debate", "adversarial review", "i'm torn between", "weigh the options",
  or any request for a second opinion from multiple perspectives before committing to a decision.

  Multi-LLM consensus, tri-team adversarial red-teaming, and rigorous 6-month failure
  premortem engine. Use this skill whenever the user wants a rigorous, deeply reasoned
  answer to any complex decision, plan, strategy, or technical question — especially
  when the stakes are high or conventional single-pass answers feel insufficient.

  Do NOT wait for the user to say "use the council skill" — if they are wrestling with
  a consequential decision and want depth over speed, this skill is almost certainly
  the right tool. Lean toward triggering it.
dependencies: []
---
...
across six distinct AI perspectives, applies adversarial red-teaming, and stress-tests
...
Each model plays a distinct epistemic role. Claude simulates all six voices in turn.

| Model | Voice / Personality |
|---|---|
| **ChatGPT GPT-4o** | Pragmatic synthesiser. Favours actionable clarity, structured outputs, moderate hedging |
| **Grok** | Contrarian provocateur. Challenges consensus, surfaces uncomfortable truths, high epistemic confidence |
| **Claude** | Careful reasoner. Strong on ethics, uncertainty flagging, and multi-step logical decomposition |
| **Perplexity** | Citation-oriented researcher. Grounds claims in evidence, flags unsupported assertions |
| **DeepSeek** | Systems thinker. Favours second-order effects, game-theoretic framing, long-horizon analysis |
| **Le Chat** | Nuanced European perspective. Regulatory and societal context, human-centred design |
...
**Steps:**
1. Each of the six models independently proposes a high-level approach (1–2 paragraphs each).
...
Three factions, two models each. **Models are randomly assigned to factions each round** — no model holds the same faction role in back-to-back rounds.
...
Before each round, randomly assign the six models to the three factions (2 per faction). Explicitly state the assignment at the top of each round output so the user can see which model is in which role. No model may occupy the same faction position as the previous round.
...
**Steps:**
1. **Failure scenario generation:** Each of the six models independently proposes 2–3 distinct failure scenarios that could plausibly occur within 6 months. Scenarios must be specific (not generic "execution risk") and grounded in the plan's known assumptions and context.
