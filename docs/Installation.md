...
4. Confirm. All 14 skills register — type `/coeus:` in chat to verify.
...
**Updates:** `/plugin update coeus` — the only true one-command update path.
...
Verify by typing `/coeus:` — you should see a popup listing all 14 skills.
...
You should see a version dir containing `skills/`, `hooks/`, `scripts/`, and `.claude-plugin/`. Then in `claude` type `/coeus:` — the popup should show all 14 skills.
