...
As of router v1.4.0 these non-trigger cases are formalized as a **Step-0 gate** with three outcomes — BYPASS (named skill: launch directly), NO ROUTE (no Coeus skill applies: say so, never force a route), PROCEED (continue to family matching). v1.4.0 also adds sequential cross-family routing (route + launch the first task, name the handoff in WHY), a three-tier confidence fallback (resolve / clarify-once / NO ROUTE), and a labeled golden set at `skills/coeus-router/references/routing-golden-set.md` that pins routing behaviour across edits (31 cases as of router v1.7.0, which adds the `atlas` row and tie-breaker 13 — the deliverable decides: *see* the project → `atlas`, *operate* the session → `project-lifecycle`, `.dugprj`/volume lineage → `dug_binary`; v1.6.0 added the `seer` routing-eval row and tie-breaker 12 — ask/explain stays NO ROUTE, test/evaluate/grade routes to `seer`; v1.5.0 added the **writing** family and tie-breaker 11). Design rationale: at 16 routable skills the catalog sits under the ~15–20-candidate accuracy-degradation threshold documented for LLM tool selection, so vector-store retrieval (RAG-MCP pattern) was evaluated and deliberately skipped — revisit if the catalog passes ~20 skills or any single family passes ~10. As of v3.20.0 that threshold claim is no longer only literature-sourced: the `seer` skill (Agent Seer protocol, arXiv 2608.26133) synthesizes routing scenarios from the skill frontmatter itself and grades the router against them, so catalog-degradation can be measured on Coeus's own catalog whenever the registry grows.

---

## The Three Tiers and Six Families

**Tiers** describe a skill's *shape* (nano, cluster, router). **Families**
describe a skill's *intent* (decision, tools, seismic, vault, writing, meta).
Every skill in Coeus has both.
...
Tier 3 — Meta-skill (router)        │ Family: vault
  └─ Routing only. NO domain logic. │   obsidian-vault
  └─ < 2,000 tokens                 │ Family: writing
                                    │   minto
                                    │ Family: meta
                                    │   coeus-router
                                    │ (* = Tier 2 cluster)
