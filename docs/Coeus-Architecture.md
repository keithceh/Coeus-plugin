...
As of router v1.4.0 these non-trigger cases are formalized as a **Step-0 gate** with three outcomes — BYPASS (named skill: launch directly), NO ROUTE (no Coeus skill applies: say so, never force a route), PROCEED (continue to family matching). v1.4.0 also adds sequential cross-family routing (route + launch the first task, name the handoff in WHY), a three-tier confidence fallback (resolve / clarify-once / NO ROUTE), and a labeled golden set at `skills/coeus-router/references/routing-golden-set.md` that pins routing behaviour across edits (27 cases as of router v1.5.0, which adds the **writing** family and tie-breaker 11). Design rationale: at 14 routable skills the catalog sits under the ~15–20-candidate accuracy-degradation threshold documented for LLM tool selection, so vector-store retrieval (RAG-MCP pattern) was evaluated and deliberately skipped — revisit if the catalog passes ~20 skills or any single family passes ~10.
